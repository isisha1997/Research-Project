...
description: Research and build a verifiable education guide for a next-gen material, where every published sentence is generated from a claim that carries its own sources. Use when asked for an education guide, explainer, or fact-checked briefing on a material (e.g. "build an education guide on mycelium leather"), or when asked to verify, fix, or update an existing guide spec in guides/. Produces a YAML claim spec that passes four automated gates - coverage, citation resolution, source tiering, and contradiction surfacing - then renders it to Markdown and HTML.
---
